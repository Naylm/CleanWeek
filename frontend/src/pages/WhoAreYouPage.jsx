...
export default function WhoAreYouPage() {
  ...
  const handleUserSelect = (slug) => {
    console.log('🔍 DEBUG: Click detected on user:', slug)
    try {
      console.log('🔍 DEBUG: Calling selectUser with slug:', slug)
      selectUser(slug)
      console.log('🔍 DEBUG: selectUser called successfully')
    } catch (error) {
      console.error('🔍 DEBUG: Error in selectUser:', error)
    }
  }
    ...
    <div className="who-page">
      ...
      <div className="who-card">
        ...
        <div className="who-buttons">
          <button className="who-btn laura" onClick={() => handleUserSelect('laura')}>
            <span className="who-avatar" style={{ background: '#FF6584' }}>L</span>
            <span className="who-name">Laura</span>
          </button>

          <button className="who-btn melvin" onClick={() => handleUserSelect('melvin')}>
